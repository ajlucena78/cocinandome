...
abstract class Service
{
    ...
    public static function cargaRef(Model $model, $propiedad, $limite = null, $inicio = 0, $soloId = false, $total = false, $criterios = null)
    {
        foreach (array_keys($model->pk) as $pk);
        $id = $model->$pk;
        if (! $id) {
            return array();
        }
        ...
        if ($total or ($soloId and $index)) {
            ...
            if ($fk->relation_type() == ManyToMany) {
                ...
                $modelFk = $fk->model();
                $modelFk = new $modelFk();
                foreach (array_keys($modelFk->pk) as $pk2);
                ...
                foreach (array_keys($modelExternal->pk) as $pk2);
                $modelExternal = null;
                $nombreModel = null;
                $modelAux = new $model();
                do {
                ...
                $sql .= ' join ' . $nombreModel . ' d2 on (d2.' . $fk->link_model() . ' = model.' . $pk2 . ' and d2.' . $pk . ' = \'' . $id . '\')';
            }
            if ($criterios !== null and is_array($criterios)) {
                foreach ($criterios as $criterio => $valor) {
                    $sql .= ', ' . $criterio = '\'' . $valor . '\'';
            ...
            if ($soloId and $index) {
                while ($registro = $consulta->lee_registro()) {
                    if (is_array($index) and count($index) > 0) {
                        ...
                        // For each element of the array 'index' a dimension is created in the returned values
                        $orden = '$registros';
                        foreach ($index as $ind) {
                            $orden .= '[$registro[\'' . $ind . '\']]';
                        }
                        ...
                        if ($campo) {
                            ...
                            // A field has been specified to load its value instead of the Model one
                            $registros[$registro[$index]] = $registro[$campo];
                        } else {
                            
                            // Only the contributed ID is loaded, or if it does not come, the primary key
                    ...
                    if ($fk and $fk->relation_type() == ManyToOne) {
                        ...
                        if ($valor === null) {
                            ...
                            if (count($propiedadesObj) > 0) {
                                ...
                                $sql .= 'model2.' . $fk->link_external_model() . ' = model.' . $fk->link_model();
    ...
    public function save(Model $model, $update = false, $transaccion = false)
    {
        ...
        if (! $update) {
            ...
            foreach ($model->propiedades_clase() as $nombre) {
                ...
                if ($fk and ($fk->relation_type() == ManyToMany or $fk->relation_type() == OneToMany)) {
                    ...
                    // Relationships of type ManyToMany and OneToMany are not taken into account
                    continue;
                }
                $valor = $model->$nombre();
                if ($fk) {
                ...
                if ($fk = $model->fk($nombre)) {
                    if (($fk->relation_type() == ManyToOne or $fk->relation_type() == OneToOne) and $valor != null) {
                        foreach (array_keys($valor->pk()) as $pk2) {
                            $valor = $valor->$pk2;
                        }
                    }
                }
        ...
        // Get the id inserted and return it if it is a new one
    ...
    public function last_insert_id()
    {
        ...
        // Gets the id if it has been generated
    ...
    public function findById($id)
    {
        ...
        $sql .= ' where true';
        foreach (array_keys($this->model->pk()) as $pk) {
            if (is_array($id)) {
    ...
    public function exist_relation(Model $model, $relacion)
    {
        ...
        foreach ($model->$relacion as $elemento) {
            ...
            $sql = 'select * from ' . $fk->model_relational() . ' where ' . $pk . ' = \'' . $id . '\' and ' . $pk2 . ' = \'' . $id2 . '\'';
            $consulta = new Consulta(self::$conexion);
            if (! $consulta->ejecuta($sql)) {
                $this->error = $consulta->error();
                return false;
    ...
    private static function sql_clase_padre(& $sql, Model $model, $id = null)
    {
        ...
        if ($clasePadre and $clasePadre != 'Model') {
            ...
            foreach (array_keys($modelPadre->pk()) as $pk);
            if ($id) {
                if (is_array($id)) {
                    $id = $id[$pk];
                }
