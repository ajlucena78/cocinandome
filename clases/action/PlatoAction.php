<?php
	...
	class PlatoAction extends Action
	{
		...
		public function __construct($servicios)
		{
			...
			if ($_GET['action'] != 'receta' and $_GET['action'] != 'recetas_publicas_por_categoria' 
					and $_GET['action'] != 'recetas_publicas_por_alimento' 
		...
		public function recetas_despensa()
		{
			...
			$this->platos = $this->platoService->platos_por_despensa($_SESSION['usuario'], null
					, $this->MAX_RECETAS, $inicio, $this->platoBuscar);
		...
		public function sug_recetas_despensa()
		{
			...
			$platos = array_slice($_SESSION['usuario']->platos_puedo_hacer(), $inicio
					, $this->MAX_SUGERENCIA_RECETAS);
		...
		public function recetas()
		{
			...
			{
				//si viene el id de usuario se muestran las recetas del mismo, siempre que sea yo o alguno 
				//de mis amigos
		...
		{
			$this->plato = new Plato(array('categoria' => new CategoriaPlato()));
		...
		public function ingredientes_form()
		{
			...
			{
				$imagen = new Imagen();
				if (!$imagen->upload_foto($_FILES['foto'], APP_ROOT . 'temp/' . $this->plato->foto
						, $this->TAM_IMAGEN_RECETA))
				{
		...
		public function guardar()
		{
			...
			$this->plato->ingredientes = array();
			//validación de los datos de la receta
			if (!isset($_POST['alimentos']) or !is_array($_POST['alimentos']) 
					or count($_POST['alimentos']) == 0)
			{
			...
			{
				require_once APP_ROOT . 'clases/util/Imagen.php';
				if (!Imagen::thumb_jpeg(APP_ROOT . 'temp/' . $this->plato->foto, APP_ROOT 
						. 'res/img/plato/' . $this->plato->foto . '.jpg', $this->TAM_IMAGEN_RECETA))
				{
				...
				if (!Imagen::thumb_jpeg(APP_ROOT . 'res/img/plato/' . $this->plato->foto . '.jpg'
						, APP_ROOT . 'res/img/plato/mini/' . $this->plato->foto . '.jpg'
						, $this->TAM_FOTO_MINI))
				{
					$this->error = 'No se ha podido mover la imagen';
					return 'success';
		...
		public function nuevo_comentario()
		{
			...
			$comentarios = $this->comentarioPlatoService->find($comentarioPlato, $this->MAX_COMENTARIOS_RECETA
					, 'fecha desc', null, null, false, 0);
		...
		public function recetas_alimento()
		{
			...
			$total = $this->platoService->total_platos_para_usuario($_SESSION['usuario'], null
					, $this->alimento);
			if ($total === false)
			{
				return 'error';
			...
			$this->platos = $this->platoService->platos_para_usuario($_SESSION['usuario'], 3, $pos, null
					, $this->alimento);
			if ($this->platos === false)
			{
				return 'error';
		...
		public function recetas_categoria()
		{
			...
			if ($_GET['id_plato'])
			{
				$excludes = array('id_plato' => $_GET['id_plato']);
			}
			else
			{
				$excludes = null;
			}
			$total = $this->platoService->total_platos_para_usuario($_SESSION['usuario'], $plato, null
					, $excludes);
			if ($total === false)
			{
				return 'error';
			...
			$this->platos = $this->platoService->platos_para_usuario($_SESSION['usuario'], 4, $pos, $plato
					, null, $excludes);
			if ($this->platos === false)
			{
				return 'error';
		...
		{
			if (isset($_GET['nombre_plato']) and $_GET['nombre_plato'] 
					and strlen(trim($_GET['nombre_plato'])) < 3)
			{
				return 'success';
			}
			...
			$total = $this->platoService->total_platos_para_usuario($_SESSION['usuario'], $this->platoBuscar
					, $this->alimento);
			if (($inicio + $this->MAX_RECETAS) < $total)
			{
				$this->verMas = true;
			}
			$this->platos = $this->platoService->platos_para_usuario($_SESSION['usuario'], $this->MAX_RECETAS
					, $inicio, $this->platoBuscar, $this->alimento);
			if ($this->platos === false)
			{
				return 'error';
			}
		...
		public function buscar_avanzado_form()
		{
			...
			if (!isset($_SESSION['categorias_alimentos']))
			{
				$this->categorias = $this->categoriaPlatoService->findAll(array('nombre_categoria')
						, 'id_categoria');
		...
		public function buscador_avanzado()
		{
			...
			if (!isset($_SESSION['categorias_alimentos']))
			{
				$categorias = $this->categoriaPlatoService->findAll(array('nombre_categoria'), 'id_categoria');
				if ($this->categorias === false)
				{
					return 'error';
				}
			...
			$total = $this->platoService->total_platos_para_usuario_avanzado($_SESSION['usuario'], $plato
					, $criterios['categorias'], $recetasAmigos);
			if (!$total)
			{
		...
		public function buscar_avanzado()
		{
			...
			if (isset($_POST['orden']) and $_POST['orden'])
			{
				...
				$orden = array($_POST['orden']);
			}
			else
			{
				$orden = array('id_plato');
			}
			...
			$this->platos = $this->platoService->platos_para_usuario_avanzado($_SESSION['usuario']
					, $this->MAX_RECETAS, $inicio, $plato, $criterios['categorias'], $recetasAmigos, $orden);
			if ($this->platos === false)
			{
				return 'error';
			}
		...
		public function comentarios()
		{
			...
			$comentarios = $this->comentarioPlatoService->find($comentarioPlato, $this->MAX_COMENTARIOS_RECETA
					, 'fecha desc', null, null, false, $inicio);
			if ($comentarios === false)
			{
				return 'error';
			}
		...
		public function elegir_cantidad_ingrediente()
		{
			...
			$this->funcion = 'popup_clases_alimentos(\\\'elegir_cantidad_ingrediente\\\'); '
					. 'actualiza_info_nutricional_receta();';
			return 'success';
		}
		...
		public function me_mola()
		{
			...
			$this->plato = $this->platoService->platos_para_usuario($_SESSION['usuario'], null, 0
					, $this->plato);
			if (!$this->plato)
			{
				return 'error';
			...
			$usuario->me_mola_plato = array($this->plato);
