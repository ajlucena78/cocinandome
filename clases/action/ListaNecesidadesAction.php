<?php
	class ListaNecesidadesAction extends Action
	{
		...
		{ 
			...
			{
				$this->clasesAlimentos = $this->alimentoClaseService->findAll(array('nombre_clase_alimento')
						, 'id_clase_alimento');
		...
		public function lista_necesidades()
		{
			...
			if ((isset($_GET['nombre_alimento']) and $_GET['nombre_alimento']) 
					or (isset($_GET['id_clase_alimento']) and ($_GET['id_clase_alimento'] 
					or $_GET['id_clase_alimento'] === '0')))
			{
				...
				foreach ($this->listaNecesidadesService->lista_usuario($_SESSION['usuario'], $plan) as $lista)
				{
					...
					if (isset($_GET['nombre_alimento']) and $_GET['nombre_alimento'] 
							and stripos($lista->alimento->nombre_alimento
							, $_GET['nombre_alimento']) === false)
					{
						$guardar = false;
					}
					//si viene la categoría si pertenece a ésta
					if ($guardar and isset($_GET['id_clase_alimento']) and ($_GET['id_clase_alimento']
							or $_GET['id_clase_alimento'] === '0') 
							and $lista->alimento->clase->id_clase_alimento 
							!= $_GET['id_clase_alimento'])
					{
						$guardar = false;
					}
					if ($guardar)
