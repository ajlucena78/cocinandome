...
function olink($action, $dest = null, $params = null, $ocultar = false, $pregunta = null, $funcion = null, $add_html = false, $ancla = null
    , $clase = null, $style = null, $title = null)
{
    $action = link_action($action, $params, true);
?><a href="<?php if ($pregunta) echo 'javascript:'; else echo $action; ?>" <?php
    if ($dest) {
?> onclick="<?php if ($pregunta)
            echo 'if (pregunta(\'' . formato_html($pregunta) . '\')) ';
    ?>carga('<?php echo $action; ?>', '<?php echo $dest; ?>', <?php
    if ($funcion)
        echo '\'' . str_replace('"', "\'", $funcion) . '\'';
    else
        echo 'null';
    ?>, null<?php if ($ocultar) { ?>, true<?php } ?><?php

    if ($add_html) {
        ?>, false, true<?php } ?><?php if ($ancla) { ?>, '<?php echo $ancla; ?>'<?php
    }
    ?>); return false;"
	<?php } ?> <?php if ($clase){echo ' class="' . $clase . '"';} ?>
	<?php
    if ($style) {
        echo ' style="' . $style . '"';
    }
...
function vlinkAjax($action, $dest = null, $params = null, $ocultar = false, $funcion = null, $add_html = false, $ancla = null)
{
    $action = link_action($action, $params, true);
